/**
 * Einfaches GPIO             // Korrekte lgpio API verwenden
            this.handle = lgpio.gpiochipOpen(0); // Chip 0 für GPIO
            console.log('🔧 GPIO Chip Handle:', this.handle);
            
            // Button Pin als Input mit Pull-Up konfigurieren - korrigierte Flags
            // lgpio Flags: SET_PULL_UP sollte verfügbar sein
            let claimResult;
            if (lgpio.SET_PULL_UP !== undefined) {
                claimResult = lgpio.gpioClaimInput(this.handle, this.buttonPin, lgpio.SET_PULL_UP);
                console.log('🔧 GPIO Claim Input mit SET_PULL_UP Result:', claimResult);
            } else {
                // Fallback ohne Pull-Up Flag
                claimResult = lgpio.gpioClaimInput(this.handle, this.buttonPin);
                console.log('🔧 GPIO Claim Input ohne Pull-Up Result:', claimResult);
                console.log('⚠️ SET_PULL_UP nicht verfügbar - externe Pull-Up erforderlich');
            } für Raspberry Pi 5
