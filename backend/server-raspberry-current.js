...
// GPIO Test Route - Manueller Button-Test
app.post('/api/gpio/test', (req, res) => {
  try {
    console.log('🔘 GPIO Test API aufgerufen');
    
    // Prüfe GPIO-Status
    const gpioStatus = gpio.getStatus ? gpio.getStatus() : { available: false, mode: 'unknown' };
    console.log('🔍 GPIO Status:', gpioStatus);
    
    // Versuche Button-Press zu simulieren
    let testResult = false;
    if (gpio.simulateButtonPress) {
      testResult = gpio.simulateButtonPress();
    }
    
    res.json({
      success: true,
      message: 'GPIO Test ausgeführt',
      gpioStatus: gpioStatus,
      testExecuted: testResult,
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    console.error('❌ GPIO Test Fehler:', error);
    res.status(500).json({
      success: false,
      message: 'GPIO Test fehlgeschlagen: ' + error.message
    });
  }
});
